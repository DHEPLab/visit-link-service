...
public class HistoryHandlerFactory {
    ...
    @Autowired
    private UserHistoryHandlerFilter userFilter;
    ...
    @Pointcut("execution(* edu.stanford.fsi.reap.repository.VisitRepository.deleteByLessonIdAndStatus(java.lang.Long,edu.stanford.fsi.reap.entity.enumerations.VisitStatus))")
    ...
    @Pointcut("execution(* edu.stanford.fsi.reap.repository.VisitRepository.deleteByBabyIdAndStatus(java.lang.Long,edu.stanford.fsi.reap.entity.enumerations.VisitStatus))")
    ...
    public void delByBabyIdRecord(JoinPoint joinPoint) {
        try {
            ...
            VisitStatus visitStatus = (VisitStatus) args[1];
            visitRepository.findByBabyIdAndStatus(babyId, visitStatus).ifPresent(visit -> {
                VisitHistory visitHistory = new VisitHistory();
                BeanUtils.copyProperties(visit, visitHistory);
                visitHistory.setHistoryId(visit.getId());
                visitHistoryRepository.save(visitHistory);
