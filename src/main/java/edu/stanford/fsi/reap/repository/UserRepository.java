...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(
            countQuery = "select count(user) from User user where user.role = 'ROLE_CHW' and user.projectId=?3 "
                    + "and (?1 is null or ?1 = '' or (user.realName like concat('%', ?1, '%') or user.chw.identity like concat('%', ?1, '%') or user.chw.tags like concat('%', ?1, '%'))) and (?2 is null or user.chw.supervisor.id = ?2) ",
            value =
                    "select new edu.stanford.fsi.reap.dto.ChwUserDTO(user, (select count(baby) from Baby baby where baby.deleted = 0 and baby.chw = user)) "
                            + "from User user where user.role = 'ROLE_CHW'  and user.projectId=?3 "
                            + "and (?1 is null or ?1 = '' or (user.realName like concat('%', ?1, '%') or user.chw.identity like concat('%', ?1, '%') or user.chw.tags like concat('%', ?1, '%'))) and (?2 is null or user.chw.supervisor.id = ?2) "
                            + "order by user.id desc")
    Page<ChwUserDTO> findChwBySearch(String search, Long supervisorId, Long projectId, Pageable pageable);

    @Query(
            countQuery = "select count(user) from User user where user.role = 'ROLE_SUPERVISOR'",
            value =
                    "select new edu.stanford.fsi.reap.dto.SupervisorUserDTO(user, (select count(chw.id) from CommunityHouseWorker chw where chw.supervisor = user)) "
                            + "from User user where user.role = 'ROLE_SUPERVISOR' and user.projectId=?1 "
                            + "order by user.id desc")
    Page<SupervisorUserDTO> findAllSupervisor(Long projectId,Pageable pageable);

    @Query(
            "select user "
                    + "from User user where user.role = 'ROLE_CHW' and user.projectId=?2 and user.chw.supervisor is null and "
                    + "(?1 is null or ?1 = '' or (user.realName like concat('%', ?1, '%') or user.chw.identity like concat('%', ?1, '%') or user.chw.tags like concat('%', ?1, '%')))")
