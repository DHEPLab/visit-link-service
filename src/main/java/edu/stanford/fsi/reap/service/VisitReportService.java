...
public class VisitReportService {
  ...
  public VisitReportService(VisitReportRepository visitReportRepository, CarerRepository carerRepository, ModuleRepository moduleRepository, QuestionnaireRecordRepository questionnaireRecordRepository, ExcelService excelService) {
  ...
  public byte[] report(ReportDTO reportDTO) {
    ...
    List<VisitReportObjData> list = visitReportRepository.findByCreatedAtBetween(reportDTO.getStartTime(), reportDTO.getEndTime()).stream()
            .filter(target->{
                return target.getVisit().getProjectId().equals(projectId)&&target.getVisitReportObjData()!=null;
            })
            .map(visitReport -> {
    ...
    private void sort(List<VisitReportObjData> list) {
        for (VisitReportObjData visitReportObjData : list) {
          visitReportObjData.setQuestionnaireRecords(visitReportObjData.getQuestionnaireRecords().stream()
                  .sorted((o1, o2) -> {
  ...
  public void updateVisitRemarkInfo(Visit visit) {
    log.info("update report by visit remark , visit id is {}, remark is {}", visit.getId(), visit.getRemark());
    visitReportRepository.findByVisitId(visit.getId())
        .ifPresent(visitReport -> {
  ...
  public VisitReport saveVisitReport(Visit visit) {
    return visitReportRepository.save(
        VisitReport.builder().visitReportObjData(mapVisitReportObjData(visit)).visit(visit).build()
    );
  }

  private VisitReportObjData mapVisitReportObjData(Visit visit) {
    log.info("write report by visit , id is {} , babyId is {} , lessonId is {},"
        , visit.getId()
        , visit.getBaby().getId()
    ...
    return VisitReportObjData.builder()
        ...
        .questionnaireRecords(questionnaireRecordRepository.findByVisitIdOrderByTitleNoAsc(visit.getId()))
        .build();
  }
  ...
  private List<Module> mapModule(List<Domain> domains) {
    ...
    return moduleRepository.findByBranchAndPublishedTrue(CurriculumBranch.MASTER)
        .stream()
