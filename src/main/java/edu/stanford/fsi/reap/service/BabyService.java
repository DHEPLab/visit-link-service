...
public class BabyService {
    ...
    public BabyService(
            ...
            UserRepository userRepository, CarerService carerService,
            LessonService lessonService,
            VisitRepository visitRepository, ExcelService excelService, ModelMapper modelMapper,
            VisitPositionRecordRepository visitPositionRecordRepository, BabyLocationHandler babyLocationHandler,
            AccountOperationRecordService accountOperationRecordService, BabyHistoryRepository historyRepository,
            BabyModifyRecordRepository babyModifyRecordRepository, BabyUpdateInfoRepository babyUpdateInfoRepository) {
    ...
    public byte[] babyRosterReport() {
        ...
        List<Baby> list = repository.findByOrderByCreatedAtDesc().stream().filter(target -> {
            return target.getProjectId().equals(projectId);
        }).collect(Collectors.toList());
        return excelService.writeBabyRoster(
                list
        );
    }
    ...
    public List<Map<String, String>> check(@Valid List<ImportBabyDto> list) {
        ...
        list.stream().forEach(baby -> {
            ...
            if (baby.getChw().getChw().getIdentity() != null) {
                boolean chwExtend = userRepository.findOneByChw_Identity(baby.getChw().getChw().getIdentity()).isPresent();
                if (!chwExtend) {
                    Map<String, String> map = new HashMap<>();
                    map.put("name", baby.getName());
                    map.put("number", baby.getNumber().toString());
    ...
    public void imports(List<ImportBabyDto> babies) {
        babies.stream().forEach(dto -> {
            Baby baby = modelMapper.map(dto, Baby.class);
            if (baby.getChw() != null && baby.getChw().getChw() != null && baby.getChw().getChw().getIdentity() != null) {
                User user = userRepository.findOneByChw_Identity(baby.getChw().getChw().getIdentity()).get();
            ...
            List<Carer> carers = dto.getCares().stream().map(carer -> {
                carer.setBaby(baby);
                return carer;
            }).collect(Collectors.toList());
    ...
    public List<AppBabyDTO> findBabiesAvailableForCreateVisit(
        ...
        List<Baby> babies =
                repository.findByChwIdAndCurriculumIdNotNullAndDeletedFalse(userId);
        List<Baby> targetBabies = new ArrayList<>();
        List<Long> babyIds = babies.stream().filter(baby -> {
            return !baby.getApproved();
        }).map(Baby::getId).collect(Collectors.toList());
        List<BabyUpdateInfo> babyUpdateInfos = babyUpdateInfoRepository.findByBabyIdInAndUpdateNormalTrueAndDeletedFalse(babyIds);
        if (CollUtil.isNotEmpty(babyUpdateInfos)) {
            List<Long> updateBabies = babyUpdateInfos.stream().map(BabyUpdateInfo::getBabyId).collect(Collectors.toList());
            babies.stream().forEach(baby -> {
                if (baby.getApproved()) {
        ...
        repository
                ...
                .ifPresent(
                        baby -> {
                            ...
                            Optional<BabyUpdateInfo> babyUpdateInfoOptional = babyUpdateInfoRepository.findByBabyIdAndDeleted(baby.getId(), false);
    ...
    private void deleteNotStartVisit(Long id) {
        ...
        List<AdminBabyVisitDTO> notStartVisit = visitDTOS.stream().filter(visit -> visit.getStatus().equals(VisitStatus.NOT_STARTED)).collect(Collectors.toList());
        for (AdminBabyVisitDTO visitDto : notStartVisit) {
            visitRepository.findById(visitDto.getId())
                    .ifPresent(
                            visit -> {
    ...
    public void approve(Baby baby) {
        babyUpdateInfoRepository.findByBabyIdAndDeleted(baby.getId(), false).ifPresent(babyUpdateInfo -> {
        ...
        if (baby.approveDelete()) {
            ...
            List<BabyModifyRecord> approvedBabyModifyRecords = babyModifyRecordRepository.findByBabyIdAndApprovedFalseOrderByLastModifiedAtDesc(baby.getId());
    ...
    public void reject(Baby baby) {
        ...
        BabyHistory lastApprovedBaby = historyRepository.findFirstByHistoryIdAndApprovedTrueOrderByLastModifiedAtDesc(baby.getId());
        ...
        if (repository.findById(id).isPresent()) {
            try {
                ...
                if (!(differentColumns.isEmpty())) {
                    ...
                    babyModifyRecordRepository.save(new BabyModifyRecord(null, SecurityUtils.getUserId(), baby.getId(), oldBaby, baby, differentColumnsStr, approved));
                }
    ...
    public void resetLocation(Long id) {
        repository.findById(id).ifPresent((baby -> {
