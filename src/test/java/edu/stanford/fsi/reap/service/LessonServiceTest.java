...
import edu.stanford.fsi.reap.utils.WithMockCustomUser;
...
class LessonServiceTest {
  ...
  public static void beforeAll() {
    ...
    when(lessonScheduleRepository.findByCurriculumIdAndStage(1L, BabyStage.EDC)).thenReturn(mockEDCStageSchedules());
    when(lessonRepository.findByCurriculumIdAndStage(1L, BabyStage.EDC)).thenReturn(mockEDCStageLessons());
    when(lessonScheduleRepository.findByCurriculumIdAndStage(1L, BabyStage.BIRTH)).thenReturn(mockBirthStageSchedules());
    when(lessonRepository.findByCurriculumIdAndStage(1L, BabyStage.BIRTH)).thenReturn(mockBirthStageLessons());
    when(visitRepository.findByLessonIdAndBabyId(11L, 1L)).thenReturn(Collections.singletonList(Visit.builder().status(VisitStatus.DONE).build()));
    when(visitRepository.findByLessonIdAndBabyId(11L, 2L)).thenReturn(Collections.singletonList(Visit.builder().status(VisitStatus.EXPIRED).build()));
    when(visitRepository.findByBabyIdAndNotStarted(12L)).thenReturn(Collections.singletonList(new VisitResultDTO() {
      @Override
      public Long getId() {
        return 12L;
      }
    ...
    service = new LessonService(lessonRepository, lessonScheduleRepository, moduleRepository, visitRepository, questionnaireRepository);
  }
  ...
  private static List<LessonSchedule> mockEDCStageSchedules() {
    return Collections.singletonList(LessonSchedule.builder()
            .stage(BabyStage.EDC)
            .startOfApplicableDays(61)
  ...
  public void should_return_app_offline_lessons() {
    ...
    assertArrayEquals(Collections.singletonList(AppOfflineLessonDTO.builder()
                    .id(10L)
                    .name("L1")
                    .description("L11")
